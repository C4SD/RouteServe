...
import { Calendar, Clock, ChevronDown, ChevronUp, SlidersHorizontal, CalendarDays, Sparkles, X, CheckCircle2 } from 'lucide-react';
...
function computeAIOptimalIntent(snapshot: CopilotStep3IntentProps['operationalSnapshot']): AIOptimizationResult {
  ...
  // High demand relative to fleet → minimize vehicles, balance utilization
  ...
  // Always respect facility hours unless critically understaffed
  const respect_facility_hours = driverUtilization < 0.9;

  // Prioritize cold-chain unless we have very few drivers (can't separate runs)
  const prioritize_cold_chain = drivers_active >= 2;

  // Shift hours: tight fleet → longer shifts; ample fleet → standard
...
export function CopilotStep3Intent({
  ...
  // Derive DateRange from string fields
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="rounded-lg border bg-muted/30 p-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            <p className="text-xs text-muted-foreground">Ready Demand</p>
      ...
      {/* Planning Window — range date picker */}
